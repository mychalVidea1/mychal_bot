...
// Destrukturace byla upravena pro přidání PermissionFlagsBits
...
async function applyTimeout(member, durationInMs, reason) {
    if (!member || !member.manageable) {
        console.error(`Nelze udělit timeout uživateli ${member.user.tag} (nemám oprávnění nebo má vyšší roli).`);
        return;
    }
    try {
...
async function analyzeImage(imageUrl) {
    ...
    try {
        ...
        if (mimeType.startsWith('image/')) { imageBuffer = await sharp(imageBuffer).resize({ width: 512, withoutEnlargement: true }).toBuffer(); } else { return false; }
    } catch (preprocessingError) { console.error("Chyba při zpracování obrázku:", preprocessingError); return 'FILTERED'; }
...
async function moderateMessage(message) {
    ...
    if (!mediaUrl && message.embeds.length > 0) {
        ...
        } else if (embed.url && (embed.type === 'image' || embed.type === 'gifv')) { // Přidána kontrola
            mediaUrl = embed.url;
        }
    }
    if (!mediaUrl) {
    ...
    const wordCount = textToAnalyze.split(' ').filter(Boolean).length;
...
async function checkRepetitiveSpam(message) {
    ...
    userHistory.set(message.id, { content: message.content, timestamp: Date.now() });

    // Vyčistit staré zprávy, které jsou starší než 10 sekund, aby se paměť nezaplňovala
    const now = Date.now();
    for (const [messageId, msgData] of userHistory.entries()) {
        if (now - msgData.timestamp > 10000) {
            userHistory.delete(messageId);
        }
    }
    ...
    if (isSpam) {
        ...
        try {
            await message.channel.bulkDelete(messagesToDelete, true); // true = filtrovat zprávy starší 14 dní
...
client.once('clientReady', async () => {
    ...
    try {
        ...
        // ================== OPRAVA OPRÁVNĚNÍ PŘÍKAZŮ ==================
        const commands = [
            // Admin příkazy
            new SlashCommandBuilder().setName('rate').setDescription('Ohodnotí uživatele (pouze pro adminy).').addUserOption(option => option.setName('uživatel').setDescription('Uživatel, kterého chceš ohodnotit.').setRequired(true)).addNumberOption(option => option.setName('hodnocení').setDescription('Číslo od 0 do 10.').setRequired(true).setMinValue(0).setMaxValue(10)).setDMPermission(false).setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
            new SlashCommandBuilder().setName('resetscoreboard').setDescription('Smaže všechna data hodnocení (pouze pro adminy).').setDMPermission(false).setDefaultMemberPermissions(PermissionFlagsBits.Administrator),
            
            // Příkazy pouze pro vlastníka bota (owner-only)
            new SlashCommandBuilder().setName('list-servers').setDescription('Vypíše seznam serverů, kde se bot nachází (pouze pro majitele).').setDMPermission(false).setDefaultMemberPermissions(PermissionFlagsBits.Administrator), // Omezit na admina pro jistotu
            new SlashCommandBuilder().setName('leave-server').setDescription('Přinutí bota opustit server podle ID (pouze pro majitele).').addStringOption(option => option.setName('id').setDescription('ID serveru, který má bot opustit.').setRequired(true)).setDMPermission(false).setDefaultMemberPermissions(PermissionFlagsBits.Administrator), // Omezit na admina pro jistotu

            // Veřejné příkazy
            new SlashCommandBuilder().setName('score').setDescription('Zobrazí tvé hodnocení nebo hodnocení jiného uživatele.').addUserOption(option => option.setName('uživatel').setDescription('Uživatel, jehož skóre chceš vidět.').setRequired(false)).setDMPermission(false), // Bez omezení = veřejný
            new SlashCommandBuilder().setName('scoreboard').setDescription('Zobrazí síň slávy - žebříček všech uživatelů.').setDMPermission(false), // Bez omezení = veřejný
        ...
        /*
            POZNÁMKA K DUPLICITNÍM PŘÍKAZŮM:
            Pokud po nasazení tohoto kódu stále vidíte duplicitní příkazy,
            je třeba jednorázově spustit skript, který vymaže všechny staré příkazy na serveru.
            Vytvořte soubor např. `clear_commands.js` a vložte do něj:
            ...
            Spusťte ho příkazem `node clear_commands.js`. Poté ho smažte a restartujte hlavního bota.
        */
    ...
    try {
        ...
        if (channel) {
            const startupEmbed = new EmbedBuilder().setColor('#00FF00').setTitle('🚀 JSEM ZPÁTKY ONLINE! 🚀').setDescription('Systémy nastartovány, databáze pročištěna. Jsem připraven hodnotit vaše chování! 👀').setImage('https://media.tenor.com/2h40b0nF0BUAAAAC/robot-ai.gif').setTimestamp().setFooter({ text: 'mychalVidea' });
...
client.on('interactionCreate', async interaction => {
    if (interaction.isButton()) {
        if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) { return interaction.reply({ content: 'K této akci nemáš oprávnění.', ephemeral: true }); }
    ...
    if (commandName === 'resetscoreboard') {
        // Kontrola oprávnění je již na definici příkazu, ale pro jistotu ji zde necháváme
        if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) { return interaction.reply({ content: 'K tomuto příkazu má přístup pouze administrátor serveru.', ephemeral: true }); }
        await interaction.deferReply({ ephemeral: true });
        ratings = {};
        messageCounts = {};
        saveRatings();
    ...
    if (commandName === 'rate') {
        ...
        ratings[user.id] = ratingInput; // Nastaví hodnocení přímo, neprůměruje
        saveRatings();
        console.log(`Uživatel ${user.id} byl ohodnocen adminem ${interaction.user.tag}. Nové skóre: ${ratingInput}.`);
        await updateRoleStatus(user.id, interaction.guild);
        await interaction.editReply({ content: `**<@${user.id}>** obdržel(a) nové hodnocení! 🔥 Nové skóre: **\`${ratingInput.toFixed(2)} / 10\`**` });
    }
    ...
    if (commandName === 'score') {
        ...
        // ephemeral flag se již nenastavuje v deferReply, ale v editReply/reply
